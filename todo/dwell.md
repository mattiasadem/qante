...
**Durum:** Mod A walk + resmi 3vp + interact + validate **devam** (pre-capture)  
...
**Kapsam satırı:**  
`Dwell · default · dwell-100-0-0 / schema_name Dwell 100.0.0 (theme_store_id null) · home→PDP→PLP→collections index→search→cart→about/contact→blog→404 · 0 yeni şema · leftover: footer-utilities / search-header / 768 hamburger / 404 routes / quick-add`
...
| Şablon | Durum | 3vp |
|---|---|---|
| Home + announcement / hero / featured / shop-the-look / collections / footer | [x] obs | [ ] capture |
| Header kapalı + Bedding mega 1440/768 | [x] obs | [ ] interact |
| Predictive search `linen` | [x] obs | [ ] interact 375/768/1440 |
| Mobile hamburger 375 (+ 768 yoklama) | [x] obs | [ ] interact |
| Cart drawer empty + filled + qty | [x] obs | [ ] interact |
| PDP Down Pillow Size Standard→King | [x] obs | [ ] interact |
| PLP `/collections/bedding-1` | [x] obs | [ ] capture |
| Collections index `/collections` | [x] obs | [ ] capture |
| Search `/search?q=linen` | [x] obs | [ ] capture |
| Cart `/cart` empty + filled + qty | [x] obs | [ ] interact |
| About `/pages/about-us` | [x] obs | [ ] capture |
| Contact form (submit yok) | [x] obs | [ ] capture |
| News list + Art of Sheet Folding | [x] obs | [ ] capture |
| FAQ + Shipping policy | [x] obs | [ ] capture |
...
| 5 | product_list_YidTcf | `product-showcase-grid-featured` | reuse · slider + Choose |
...
- Header 1440/768 Bedding hover — mega grid beklenir
- Hamburger 375: nav drawer; 768 hamburger 0px leftover
- Predictive `linen`: 75 sonuç kataloğu
- Cart drawer: empty → Down Pillow qty 1 $250 → qty 2 $500
- Cart page: empty → filled → qty
- PDP Size: Standard → King
