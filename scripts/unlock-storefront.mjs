...
export async function gotoAndUnlock(page, url, password) {
  ...
  if (now.pathname.replace(/\/$/, "") !== target.pathname.replace(/\/$/, "")) {
    await page.goto(url, { waitUntil: "domcontentloaded", timeout: 90000 });
...
export function passwordFromObservation(obs, url) {
  ...
  if (obs?.capture?.storefrontPassword) return String(obs.capture.storefrontPassword);
  try {
    const host = new URL(url || obs?.url || "").hostname;
    if (host === "women-beauty-8.myshopify.com") return "1";
  } catch {
    /* ignore */
  }
  return null;
