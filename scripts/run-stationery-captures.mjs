/**
 * Stationery Mod A static 3vp captures.
 */
...
function findObs() {
  const base = path.join(root, "observations/stationery/default");
  const out = [];
  for (const dir of fs.readdirSync(base)) {
    const p = path.join(base, dir);
    if (!fs.statSync(p).isDirectory()) continue;
    for (const f of fs.readdirSync(p)) {
      if (f.endsWith(".json")) out.push(`observations/stationery/default/${dir}/${f}`);
    }
  }
  return out.sort();
}
...
const observations = only.length ? only : findObs();
const results = [];

for (const rel of observations) {
  const abs = path.isAbsolute(rel) ? rel : path.join(root, rel);
  const obs = JSON.parse(fs.readFileSync(abs, "utf8"));
  if (obs.interactionSteps?.length && !obs.capture?.method?.includes("margin")) {
    // still capture initial static if no 3vp yet
  }
  ...
  if (existing.length >= 3) {
    ...
    process.stdout.write(`skip ${rel} (already ${existing.length} vp)\n`);
    continue;
  }
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES && !captured; attempt++) {
    ...
    try {
      ...
      if (attempt === MAX_RETRIES)
        results.push({ rel, ok: false, error: e.message?.slice(0, 220) });
    }
  }
