...
const obsRoot = path.join(root, 'observations/obvi/default');
...
async function clearCart() {
  ...
  await page.goto('https://myobvi.com/', { waitUntil: 'domcontentloaded', timeout: 120000 });
...
for (const abs of obsFiles) {
  ...
  const hasSteps = Array.isArray(obs.interactionSteps) && obs.interactionSteps.length > 0;
  const script = hasSteps ? 'capture-interaction.mjs' : 'capture-observation.mjs';

  if (obs.schemaId === 'global-cart-drawer') {
